...
template <class T> Matrix<Polynomial> matrixToPolynomial(Matrix<T> m) {
  if (!isArithmetic<T>()) {
    throw std::invalid_argument(
        "Only matrices of arithmetic types can be converted to polynomial");
  }
...
template <class T> Matrix<T>::Matrix(int h, int w, std::vector<T> m) {
  height = h;
  width = w;
  mat = std::move(m);
...
template <class T> Matrix<T>::Matrix(int h, int w) {
  height = h;
  width = w;
...
template <class T> T Matrix<T>::determinant() {
  if (height != width) {
    throw std::invalid_argument(
        "Determinant can only be calculated of square matrix");
  } else if (height == 1)
    return mat[0];
...
template <class T> std::vector<std::vector<T>> Matrix<T>::generateNested() {
...
template <class T> std::vector<std::vector<T>> Matrix<T>::columns() {
  std::vector<std::vector<T>> ret = {};
...
template <class T> Matrix<T> Matrix<T>::operator+(const Matrix<T> &obj) {
...
template <class T> Matrix<T> Matrix<T>::operator-(const Matrix<T> &obj) {
  if (height != obj.height || width != obj.width) {
    throw std::invalid_argument(
        "Matrices must be the same dimensions to subtract");
  }
  std::vector<T> result = obj.mat;

  for (int i = 0; i < result.size(); i++) {
    result[i] = mat[i] - result[i];
  }
...
template <class T> Matrix<T> Matrix<T>::r(int i, int j) {
  if (i < 1 || j < 1 || i > height || j > height) {
    throw std::invalid_argument("Rows out of bounds for matrix " +
...
template <class T> Matrix<T> Matrix<T>::r(int i, T lambda) {
  if (i < 1 || i > height)
    throw std::invalid_argument("Row out of bounds for matrix " +
                                std::to_string(i));
...
template <class T> Matrix<T> Matrix<T>::r(int i, int j, T lambda) {
  if (i < 1 || j < 1 || i > height || j > height) {
    throw std::invalid_argument("Rows out of bounds for matrix " +
                                std::to_string(i) + " " + std::to_string(j));
  }
...
template <class T> void Matrix<T>::insert(T e, int i, int j) {
  int ind = (i - 1) * width + j - 1;
...
template <class T> Matrix<T> Matrix<T>::augment(ColumnVector<T> v) {
  if (v.height != height) {
    throw std::invalid_argument(
        "Can only augment matrix and vector of the same height");
  }
  std::vector<T> newMat = {};
  std::vector<std::vector<T>> nest = generateNested();
...
template <class T> bool Matrix<T>::isZero() {
  for (auto element : mat) {
    if (element != additiveIdentity<T>())
      return false;
  }
  return true;
}

template <class T> std::pair<int, T> Matrix<T>::leadingEntry(int row) {
  std::vector<T> rw = generateNested()[row - 1];

  for (int i = 0; i < rw.size(); i++) {
    if (rw[i] != additiveIdentity<T>())
      return {i + 1, rw[i]};
  }
...
template <class T> Matrix<double> Matrix<T>::forwardPhase() {
...
template <class T> Matrix<double> Matrix<T>::backwardPhase() {
...
template <class T> Matrix<double> Matrix<T>::rowReduce() {
  if (!isArithmetic<T>()) {
    throw std::invalid_argument(
        "Can only row reduce a matrix of arithmetic type");
  }
...
template <class T> bool Matrix<T>::operator==(const Matrix<T> &obj) const {
  bool checker = true;
  if (height != obj.height || width != obj.width) {
    return false;
  }
...
template <class T> bool Matrix<T>::operator!=(const Matrix<T> &obj) {
  return !(*this == obj);
}

template <class T> bool Matrix<T>::isDiagonal() {
  for (int i = 1; i <= height; i++) {
    for (int j = 1; j <= width; j++) {
      if (i != j && element(i, j) != additiveIdentity<T>())
        return false;
    }
  }
...
template <class T> Matrix<double> Matrix<T>::inverse() {
  if (!isArithmetic<T>()) {
    throw std::invalid_argument(
        "Can only calculate inverse of matrix of arithmetic type");
  }
...
template <class T> bool Matrix<T>::isSquare() { return height == width; }

// Shaowy05: Implementing upper and lower triangular functions
template <class T> bool Matrix<T>::isUpperTriangular() {
  if (!isSquare())
    return false;
  ...
  for (int i = 2; i <= height; i++) {
    for (int j = 1; j <= i - 1; j++) {
      if (element(i, j) != additiveIdentity<T>())
...
template <class T> bool Matrix<T>::isStrictlyUpperTriangular() {
...
template <class T> bool Matrix<T>::isLowerTriangular() {
  Matrix<T> t = this->transpose();
  return t.isUpperTriangular();
}

template <class T> bool Matrix<T>::isStrictlyLowerTriangular() {
  Matrix<T> t = this->transpose();
  return t.isStrictlyUpperTriangular();
}

template <class T> bool Matrix<T>::isOrthogonal() {
...
template <class T> bool Matrix<T>::isOrthonormal() {
  std::set<ColumnVector<T>> cs;
  for (auto col : columns()) {
    cs.insert(ColumnVector<T>(col));
  }
...
template <class T> bool Matrix<T>::isSymmetric() {
  return (*this) == transpose();
}

template <class T> bool Matrix<T>::isEigenvalue(double lambda) {
  if (!isArithmetic<T>()) {
    throw std::invalid_argument(
        "Can only check for eigenvalue if of arithmetic type");
  }
...
template <class T> std::vector<std::complex<double>> Matrix<T>::eigenvalues() {
  if (!isSquare()) {
    throw std::invalid_argument("Only square matrices have eigenvalues");
  }
  if (!isArithmetic<T>()) {
    throw std::invalid_argument(
        "Eigenvalues can only be calculated for matrix of arithmetic type");
  }

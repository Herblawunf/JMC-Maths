...
template <class T> Matrix<T> operator*(Matrix<T> mat, ColumnVector<T> v) {
  return mat * v.toMatrix();
}

template <class T> Matrix<T> operator*(ColumnVector<T> v, Matrix<T> mat) {
  return v.toMatrix() * mat;
}

template <class T> Matrix<T> operator*(T lambda, Matrix<T> m) {
  return m * lambda;
}

template <class T> inline double dotP(ColumnVector<T> c1, ColumnVector<T> c2) {
  if (c1.height != c2.height) {
    throw std::invalid_argument(
        "Vectors with different heights cannot have a dot product");
  }
  if (!std::is_arithmetic_v<T>) {
    throw std::invalid_argument(
        "Dot product can only be taken of vectors of arithmetic type");
  }
  double dot = 0;
  for (int i = 0; i < c1.height; i++)
    dot += c1.vector[i] * c2.vector[i];
  return dot;
}

template <class T> ColumnVector<T> operator*(T lambda, ColumnVector<T> c) {
  return c * lambda;
}
